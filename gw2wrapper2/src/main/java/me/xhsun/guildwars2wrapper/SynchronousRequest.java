...
import me.xhsun.guildwars2wrapper.model.v2.guild.GuildRank;
import me.xhsun.guildwars2wrapper.model.v2.guild.GuildStash;
...
public class SynchronousRequest extends Request {
	...
	public List<Integer> getCharacterQuests(String API, String name) throws GuildWars2Exception {
		isParamValid(new ParamChecker(ParamType.API, API), new ParamChecker(ParamType.CHAR, name));
		try {
			Response<List<Integer>> response = gw2API.getCharacterQuests(name, API).execute();
			if (!response.isSuccessful()) throwError(response.code(), response.errorBody());
			return response.body();
		} catch (IOException e) {
			throw new GuildWars2Exception(ErrorCode.Network, "Network Error: " + e.getMessage());
	...
	public List<Integer> getAllQuestIds() throws GuildWars2Exception {
		try {
			Response<List<Integer>> response = gw2API.getAllQuestIds().execute();
			if (!response.isSuccessful()) throwError(response.code(), response.errorBody());
			return response.body();
		} catch (IOException e) {
			throw new GuildWars2Exception(ErrorCode.Network, "Network Error: " + e.getMessage());
	...
	public Quest getQuestInfo(Integer id) throws GuildWars2Exception {
		try {
			Response<Quest> response = gw2API.getQuestInfo(id, null, null, null).execute();
			if (!response.isSuccessful()) throwError(response.code(), response.errorBody());
			return response.body();
		} catch (IOException e) {
			throw new GuildWars2Exception(ErrorCode.Network, "Network Error: " + e.getMessage());
	...
	public List<Quest> getQuestInfo(int[] ids) throws GuildWars2Exception {
		try {
			Response<List<Quest>> response = gw2API.getQuestInfo(null, null, null, null, processIds(ids)).execute();
			if (!response.isSuccessful()) throwError(response.code(), response.errorBody());
			return response.body();
		} catch (IOException e) {
			throw new GuildWars2Exception(ErrorCode.Network, "Network Error: " + e.getMessage());
