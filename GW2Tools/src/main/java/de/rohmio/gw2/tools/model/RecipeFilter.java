...
public class RecipeFilter {
	...
	/*
	 * TODO implement public void addItemNameFilter(ObservableStringValue itemName)
	 * { Item item =
	 * Data.getInstance().getItems().getById(recipe.getOutputItemId());
	 * this.itemName.bind(Bindings.createBooleanBinding(() ->
	 * item.getName().contains(itemName.get()), itemName)); }
	 */

	// TODO figure out how "containsFlag" constant can be set, so that it doesn't
	...
	public void addLearnedFromItemFilter(ObservableBooleanValue learnedFromItem) {
		...
		this.learnedFromItem.bind(Bindings
				.createBooleanBinding(() -> !containsFlag || containsFlag && learnedFromItem.get(), learnedFromItem));
	}
	...
	public void addAutoLearnedFilter(ObservableBooleanValue autoLearned) {
		...
		this.autoLearned.bind(
				Bindings.createBooleanBinding(() -> !containsFlag || containsFlag && autoLearned.get(), autoLearned));
	}

	public void addDiscoverableFilter(ObservableBooleanValue discoverable) {
		boolean empty = recipe.getFlags().isEmpty();
		this.discoverable
		.bind(Bindings.createBooleanBinding(() -> !empty || empty && discoverable.get(), discoverable));
	}
